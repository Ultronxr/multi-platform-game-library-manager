...
> 最后更新：2026-03-01 11:46:07 +08:00
...
- 当前环境存在 `spawn EPERM` 限制，`vite build` 可能失败；已通过 `vue-tsc` 类型校验。
- 库存筛选目前为前端内存过滤，数据量极大时可能需要后端分页与服务端过滤优化。
...
- 可考虑在后端增加库存查询筛选参数与分页接口，提升大数据量下的查询性能。
